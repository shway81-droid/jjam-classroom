...
// ── 계기일마다 전용 영상이 있는가 ────────────────────────────────
// app.js 는 계기 전용 영상(occasions 태그)이 하나도 없으면 그 계기의 topic
// 영상 전체를 대신 보여 준다. 화면이 비지는 않지만 '맞는 것이 뜬다'는 뜻은
// 아니다 — 실제로 물의 날에 분리배출·미세먼지 영상이 뜬 적이 있다.
// 편수만 보고 넘어가지 않도록, 전용 영상이 없는 계기일을 여기서 알린다.
{
  const tagged = new Set();
  for (const v of videos) for (const o of (v.occasions || [])) tagged.add(o);
  for (const name of CALENDAR_NAMES) {
    if (!tagged.has(name)) {
      warn(`계기 '${name}' 에 전용 영상(occasions 태그)이 없습니다 — 그 계기의 주제 영상 ` +
           `전체가 대신 뜨므로, 계기와 무관한 영상이 섞일 수 있습니다.`);
    }
  }
}
