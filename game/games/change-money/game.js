/* games/change-money/game.js — 패턴 A (4지선다 동시반응) — 거스름돈 */
'use strict';
...
// ── Problem Data: 낸 돈(pay) - 물건값(price) = 거스름돈(change) ─
// pay ∈ {500,1000,2000,5000}, price는 100원 단위 → 거스름돈도 100원 단위.
const ALL_WORDS = (function () {
  const pays = [500, 1000, 2000, 5000];
  const out = [];
  pays.forEach(pay => {
    for (let price = 100; price <= pay - 100; price += 100) {
      const change = pay - price;
      out.push({
        pay, price, change,
        prompt: `<span class="cm-pay">${fmt(pay)}</span> 내고<br><span class="cm-price">${fmt(price)}</span>짜리를 샀어요`,
        answer: fmt(change),
        label: `${fmt(pay)} − ${fmt(price)}`,
      });
    }
  });
  return out; // 4 + 9 + 19 + 49 = 81문제 풀
})();
...
// 4지선다: 정답 거스름돈 + 그럴듯한 오답 금액 3개 (모두 서로 다른 값)
function makeChoices(problem) {
  const correct = problem.change;
  const cand = new Set();
  [100, 200, 300, 400].forEach(d => { cand.add(correct - d); cand.add(correct + d); });
  cand.add(problem.price);     // 물건값과 헷갈리는 오답
  cand.add(problem.pay);       // 낸 돈과 헷갈리는 오답
  const pool = [...cand].filter(v => v > 0 && v !== correct);

  const wrong = shuffle(pool).slice(0, 3);
  let extra = correct + 500;
  while (wrong.length < 3) {
    if (extra > 0 && extra !== correct && !wrong.includes(extra)) wrong.push(extra);
    extra += 100;
  }
  return shuffle([correct, ...wrong]).map(fmt);
}
...
function handleAnswerTap(playerIdx, chosenName, btn) {
  ...
  const correct = (chosenName === currentWord.answer);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentWord.answer) {
  ...
  problemStatus.textContent = `${winnerLabel} 정답! (${currentWord.answer})`;
...
function handleTimeout() {
  ...
  problemStatus.textContent = `시간 초과! 정답: ${currentWord.answer}`;
...
function loadRound() {
  ...
  wordDisplay.innerHTML = currentWord.prompt;
...
function showResult() {
  ...
  roundLog.forEach((log, idx) => {
    ...
    let cells = `<td style="text-align:left;font-size:0.8rem;">${idx + 1}. ${log.label} = ${log.answer}</td>`;
