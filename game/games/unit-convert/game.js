/* games/unit-convert/game.js — 패턴 A (4지선다 동시반응) — 길이 단위 바꾸기 */
'use strict';
...
// ── Problem Data: 길이 단위 환산 (mm · cm · m) ───────────────
function buildProblems() {
  const list = [];
  const add = (word, a, unit) => list.push({ word, opposite: `${a} ${unit}`, a, unit });
  // cm → mm  (×10)
  [2, 3, 4, 5, 6, 7, 8, 9, 12, 15, 20, 25].forEach(n => add(`${n} cm = ? mm`, n * 10, 'mm'));
  // mm → cm  (÷10)
  [20, 30, 40, 50, 60, 70, 80, 90].forEach(n => add(`${n} mm = ? cm`, n / 10, 'cm'));
  // m → cm   (×100)
  [2, 3, 4, 5, 6, 8].forEach(n => add(`${n} m = ? cm`, n * 100, 'cm'));
  // cm → m   (÷100)
  [200, 300, 500, 700].forEach(n => add(`${n} cm = ? m`, n / 100, 'm'));
  return list;
}
const ALL_WORDS = buildProblems();
...
// 4지선다: 정답(환산값) + 값이 다른 오답 3개 (단위는 같게, 값만 다르게)
function makeChoices(currentPair) {
  const correct = currentPair.opposite;
  const c = currentPair.a;
  const unit = currentPair.unit;
  const step = c >= 100 ? 100 : (c >= 10 ? 10 : 1);
  const pool = [];
  const push = v => { if (v > 0 && v !== c && !pool.includes(v)) pool.push(v); };
  push(c + step); push(c - step);
  push(c + step * 2); push(c - step * 2);
  push(c * 10); push(Math.round(c / 10));
  push(c * 2); push(c + 1); push(Math.max(1, c - 1));
  const wrong = shuffle(pool).slice(0, 3).map(v => `${v} ${unit}`);
