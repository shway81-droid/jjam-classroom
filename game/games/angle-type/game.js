...
// ── Angle SVG builder ────────────────────────────────────────
// Draws two rays from a vertex with the given opening angle (degrees).
// The degree number is NOT shown — players must judge the angle by sight.
function buildAngleSVG(deg, isRight) {
  const vx = 100, vy = 110, L = 82, r = 30;
  const rad = deg * Math.PI / 180;
  const e2x = (vx + L * Math.cos(rad)).toFixed(1);
  const e2y = (vy - L * Math.sin(rad)).toFixed(1);
  const ax  = (vx + r * Math.cos(rad)).toFixed(1);
  const ay  = (vy - r * Math.sin(rad)).toFixed(1);

  let marker;
  if (isRight) {
    // Small square marks a right angle unmistakably.
    const s = 20;
    marker = `<polyline points="${vx + s},${vy} ${vx + s},${vy - s} ${vx},${vy - s}" `
           + `fill="none" stroke="#EF6C00" stroke-width="4" stroke-linejoin="round"/>`;
  } else {
    marker = `<path d="M ${vx + r} ${vy} A ${r} ${r} 0 0 0 ${ax} ${ay}" `
           + `fill="none" stroke="#EF6C00" stroke-width="4"/>`;
  }

  return `<svg viewBox="0 0 200 130" width="200" height="130" role="img">`
       + `<rect x="3" y="3" width="194" height="124" rx="14" fill="#FFF8E1" stroke="#2C2C2C" stroke-width="2.5"/>`
       + `<line x1="${vx}" y1="${vy}" x2="${vx + L}" y2="${vy}" stroke="#2C2C2C" stroke-width="5" stroke-linecap="round"/>`
       + `<line x1="${vx}" y1="${vy}" x2="${e2x}" y2="${e2y}" stroke="#2C2C2C" stroke-width="5" stroke-linecap="round"/>`
       + marker
       + `<circle cx="${vx}" cy="${vy}" r="5" fill="#2C2C2C"/>`
       + `</svg>`;
}

// ── Problem generation ───────────────────────────────────────
const ANGLE_TYPES = ['예각', '직각', '둔각', '평각'];

function generateQuestion() {
  // Pick a type, then a clearly-categorizable angle (away from 90/180 boundaries).
  const type = ANGLE_TYPES[rand(0, 3)];
  let deg;
  if (type === '예각')      deg = rand(20, 80);
  else if (type === '직각') deg = 90;
  else if (type === '둔각') deg = rand(100, 170);
  else                      deg = 180; // 평각

  const svg     = buildAngleSVG(deg, type === '직각');
  const answer  = type;
  const expr    = `${deg}° = ?`;                 // used only in the result table
  const choices = shuffle(ANGLE_TYPES.slice());  // always the 4 distinct labels

  return { expr, answer, choices, svg };
}
...
function nextQuestion() {
  ...
  problemExpr.innerHTML = currentQuestion.svg;
