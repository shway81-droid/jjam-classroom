...
function makeWrong(answer, step) {
  const cand = shuffle([
    ...
    answer + step, answer - step, answer + 3, answer - 3,
  ]);
  const out = [];
  for (const c of cand) {
...
function buildProblem() {
  ...
  if (kind === 0) {
    ...
    const d     = [1, 2, 3, 4, 5, 10][randInt(0, 5)];
    ...
    for (let i = 0; i < 4; i++) terms.push(start * Math.pow(2, i));
    answer = start * Math.pow(2, 4);
    step   = Math.floor(answer / 2);
  } else {
    // 커지는 덧셈: +1,+2,+3,+4 → 다음 +5
    const start = randInt(1, 5);
    let v = start;
    terms.push(v);
    for (let i = 1; i <= 4; i++) { v += i; terms.push(v); }
    answer = v + 5;
    step   = 5;
  }

  const display = terms.join(',  ') + ',  ?';
  const wrong   = makeWrong(answer, step);
