...
function renderBlocksSVG(tens, ones) {
  const c = 11;          // 정육면체 한 변
  const fill = '#80CBC4', stroke = '#00695C';
  let cubes = '';
  const cube = (x, y) =>
    `<rect x="${x}" y="${y}" width="${c}" height="${c}" rx="2" fill="${fill}" stroke="${stroke}" stroke-width="2"/>`;

  // 십 묶음: 막대마다 세로 10칸. 막대 사이 간격을 둔다.
  const rodGap = 6;
  const rodW   = c;
  const baseY  = 8;            // 막대 상단
  for (let t = 0; t < tens; t++) {
    const x = 6 + t * (rodW + rodGap);
    for (let k = 0; k < 10; k++) cubes += cube(x, baseY + k * c);
  }
  // 낱개: 십 묶음 오른쪽에 5칸씩 2열(아래부터 채움)로 배치
  const onesX0 = 6 + tens * (rodW + rodGap) + 12;
  for (let o = 0; o < ones; o++) {
    const col = Math.floor(o / 5);
    const rowFromBottom = o % 5;
    const x = onesX0 + col * (c + rodGap);
    const y = baseY + (9 - rowFromBottom) * c;   // 막대 바닥과 정렬
    cubes += cube(x, y);
  }
  const width  = onesX0 + (ones > 0 ? (Math.ceil(ones / 5)) * (c + rodGap) : 0) + 6;
  const height = baseY + 10 * c + 8;
  return `<svg class="blocks-svg" viewBox="0 0 ${Math.max(width, 90)} ${height}" `
       + `width="${Math.max(width, 90)}" height="${height}" xmlns="http://www.w3.org/2000/svg" `
...
function generateQuestion() {
  ...
  // 오답: 흔한 오개념 위주 — 자리 바꿔 읽기, 십/일 한 묶음 더/덜, 묶음·낱개 더해 읽기
  const cand = [];
  if (ones > 0 && tens <= 9) cand.push(ones * 10 + tens); // 자리 바꿔 읽기
  cand.push(answer + 10, answer - 10, answer + 1, answer - 1, tens + ones);
  shuffle(cand);

  const wrongSet = new Set();
  for (const w of cand) {
    if (wrongSet.size >= 3) break;
    if (w !== answer && w >= 0 && w <= 120 && !wrongSet.has(w)) wrongSet.add(w);
  }
  while (wrongSet.size < 3) {
    const w = answer + rand(1, 9) * (Math.random() < 0.5 ? 1 : -1);
    if (w !== answer && w >= 0 && !wrongSet.has(w)) wrongSet.add(w);
  }

  const choices = shuffle([answer, ...wrongSet]);
  const display = renderBlocksSVG(tens, ones);
  const expr    = `${tens}묶음 ${ones}개`;   // 결과 표용 텍스트
...
function nextQuestion() {
  ...
  problemExpr.innerHTML = currentQuestion.display;
