...
// 나눌 물건 이모지 (한 문제 안에서는 한 종류만 사용)
const ITEMS = ['🍪', '🍬', '🍎', '🍓', '🌰', '🍊', '🫐', '🍩'];
// 나눠 담을 곳 (emoji + 질문 표현)
const CONTAINERS = [
  { emoji: '🧺', per: '바구니', unit: '개' },
  { emoji: '🍽️', per: '접시',   unit: '개' },
  { emoji: '📦', per: '상자',   unit: '개' },
  { emoji: '🧒', per: '친구',   unit: '명' },
];
...
let scores       = [];
let questionLog  = [];
let currentQuestion = null;      // { html, label, answer, choices }
let dqSet        = new Set();
let phase        = 'idle';
...
// ── Question generation (clean division) ─────────────────────
function generateQuestion() {
  const extra = Math.floor((questionIdx - 1) / 4);

  let groups, quotient;
  if (difficulty === 'easy') {
    groups   = rand(2, 3);
    quotient = rand(2, Math.min(9, 4 + extra)); // grows over rounds
  } else {
    groups   = rand(2, 5);
    quotient = rand(2, Math.min(9, 6 + extra));
  }
  const total  = groups * quotient;   // dividend (always divisible)
  const answer = quotient;

  const item = ITEMS[rand(0, ITEMS.length - 1)];
  const cont = CONTAINERS[rand(0, CONTAINERS.length - 1)];

  // Wrong choices: near the answer (common errors), distinct, >= 1
  const pool = [];
  for (let n = Math.max(1, answer - 3); n <= answer + 3; n++) {
    if (n !== answer) pool.push(n);
  }
  shuffle(pool);
  const choices = shuffle([answer, pool[0], pool[1], pool[2]]);

  const html =
    `<div class="share-top"><span class="share-item">${item}</span>` +
    `<span class="share-total">${total}<span class="share-unit">개</span></span></div>` +
    `<div class="share-mid">${cont.emoji} ${groups}${cont.unit} ${cont.per}에 똑같이</div>` +
    `<div class="share-q">한 ${cont.per}에 몇 개?</div>`;

  const label = `${total} ÷ ${groups}`;

  return { html, label, answer, choices };
}
...
function showResult() {
  ...
  if (winners.length === 1) {
    ...
    resultWinner.style.color = '#EF6C00';
  }
  ...
  resultTableBody.innerHTML = questionLog.map((q, ri) => {
    ...
    return `<tr>
      <td>${ri + 1}</td>
      <td style="font-size:0.85rem;white-space:nowrap">${q.label}</td>
