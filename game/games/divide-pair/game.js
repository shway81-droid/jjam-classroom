...
(function () {
  ...
  function makeProblem() {
    for (var attempt = 0; attempt < 300; attempt++) {
      var T = rndInt(2, 9);
      var d = rndInt(2, 9);
      var D = T * d;                      // 의도된 정답: D ÷ d = T
      var p2 = [d];
      while (p2.length < 4) { var x = rndInt(2, 9); if (p2.indexOf(x) === -1) p2.push(x); }
      var p1 = [D];
      while (p1.length < 4) { var y = rndInt(4, 81); if (p1.indexOf(y) === -1) p1.push(y); }
      // 4x4 격자에서 정답 쌍의 개수를 센다
      var valid = 0;
      for (var i = 0; i < p1.length; i++) for (var j = 0; j < p2.length; j++) {
        if (p2[j] !== 0 && p1[i] % p2[j] === 0 && (p1[i] / p2[j]) === T) valid++;
      }
      if (valid === 1) return { target: T, p1: p1.slice(), p2: p2.slice() };
    }
    return { target: 3, p1: [12, 5, 7, 19], p2: [4, 3, 5, 7] }; // 안전 폴백 (12/4=3 유일)
  }
  ...
  function checkAnswer() {
    ...
    var ok = (b !== 0) && (a % b === 0) && (a / b === problem.target);
    ...
    if (ok) {
      ...
      showBanner('성공! ' + a + ' ÷ ' + b + ' = ' + problem.target, 'ok');
      sounds.play('correct');
    } else {
      p1Card.classList.add('wrong');
      p2Card.classList.add('wrong');
      if (b !== 0 && a % b === 0) {
        showBanner('아쉬워요! ' + a + ' ÷ ' + b + ' = ' + (a / b) + ' (목표 ' + problem.target + ')', 'ng');
      } else {
        showBanner('아쉬워요! ' + a + ' ÷ ' + b + ' 은 딱 나누어지지 않아요 (목표 ' + problem.target + ')', 'ng');
      }
      sounds.play('wrong');
    }
  ...
  function initGame() {
    ...
    locked = false;
    problems = [];
    for (var k = 0; k < TOTAL_ROUNDS; k++) problems.push(makeProblem());
    updateScoreUI();
    nextRound();
    showScreen('game');
  }
