...
// 후반엔 보기 4개 + 범위 1~99 + 값이 서로 가까워져 비교가 까다로움.
const DIFFICULTY = [
...
// 문제 생성: 보기 중 정확히 하나가 '유일한 최댓값'(동률 없음)이 되도록 보장.
// 반환: { values: number[], maxIdx: number }
function btGeneratePrompt(diff) {
  ...
  while (true) {
    ...
    // 최댓값이 유일한지 + 2등과 minGap 이상 떨어졌는지 확인
    const sorted = values.slice().sort((a, b) => b - a);
    const top = sorted[0];
    const second = sorted[1];
    const uniqueTop = top !== second;            // 동률 없음
    const gapOk = (top - second) >= diff.minGap; // 충분히 떨어짐
    if (uniqueTop && gapOk) break;
    if (attempts > 200) {
      // 안전망: 강제로 유일 최댓값 구성 (실제로는 거의 도달 안 함)
      values = [];
  ...
  return { values: values, maxIdx: maxIdx };
}
...
(function() {
  ...
  if (el) {
    el.innerHTML = `<svg viewBox="0 0 220 110" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="85" y="32" width="50" height="46" rx="10" fill="#A5D6A7" stroke="#2E7D32" stroke-width="3"/>
      <text x="110" y="64" text-anchor="middle" font-size="22" font-weight="900" fill="#1B5E20">58</text>
      <text x="110" y="22" text-anchor="middle" font-size="13" font-weight="900" fill="#EC407A">가장 큰 수!</text>
...
function btStartGame() {
  ...
  btBuildZones();
  btBuildScoreBar();
  btQuestionCounter.textContent = '가장 큰 수를 터치!';
