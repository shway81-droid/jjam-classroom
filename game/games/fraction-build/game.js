/* games/fraction-build/game.js — 패턴 D (협력 조합)
   그림 분수를 P1(분자=색칠된 칸)과 P2(분모=전체 칸)가 함께 만든다.
   sum-relay 골든 템플릿 기반. */
...
(function () {
  ...
  // 약분된 값은 보기에서 제외해 모호성을 없앴다(전부 기약분수).
  var PROBLEM_POOL = [
    { num: 1, den: 2, p1: [1, 2, 3, 4], p2: [2, 3, 5, 7] },
    { num: 1, den: 3, p1: [1, 2, 3, 4], p2: [3, 2, 4, 5] },
    { num: 2, den: 3, p1: [2, 1, 3, 4], p2: [3, 2, 4, 5] },
    { num: 1, den: 4, p1: [1, 2, 3, 4], p2: [4, 3, 5, 6] },
    { num: 3, den: 4, p1: [3, 2, 4, 1], p2: [4, 3, 5, 6] },
    { num: 1, den: 5, p1: [1, 2, 3, 4], p2: [5, 4, 6, 3] },
    { num: 2, den: 5, p1: [2, 3, 1, 4], p2: [5, 4, 6, 3] },
    { num: 3, den: 5, p1: [3, 4, 2, 5], p2: [5, 6, 4, 3] },
    { num: 4, den: 5, p1: [4, 3, 5, 2], p2: [5, 6, 4, 7] },
    { num: 1, den: 6, p1: [1, 2, 3, 4], p2: [6, 5, 7, 4] },
    { num: 5, den: 6, p1: [5, 4, 6, 3], p2: [6, 5, 7, 8] },
    { num: 3, den: 8, p1: [3, 4, 2, 5], p2: [8, 6, 7, 9] },
    { num: 5, den: 8, p1: [5, 6, 4, 3], p2: [8, 7, 9, 6] },
    { num: 7, den: 8, p1: [7, 6, 8, 5], p2: [8, 7, 9, 6] },
    { num: 3, den: 7, p1: [3, 2, 4, 5], p2: [7, 6, 8, 5] },
    { num: 5, den: 7, p1: [5, 6, 4, 3], p2: [7, 8, 6, 9] }
  ];
  ...
  var p1Selected;     // 선택된 카드 {idx,val} 또는 null
  var p2Selected;
  var locked;         // 채점 중 잠금
  ...
  var targetPie    = document.getElementById('targetPie');
  ...
  function renderPie(num, den) {
    var cx = 48, cy = 48, r = 44;
    var step = 360 / den;
    var svg = '<svg viewBox="0 0 96 96" width="96" height="96">';
    for (var i = 0; i < den; i++) {
      var fill = i < num ? SHADED : EMPTY;
      svg += '<path d="' + wedgePath(cx, cy, r, i * step, (i + 1) * step) +
             '" fill="' + fill + '" stroke="#2C2C2C" stroke-width="2.5"/>';
    }
    svg += '<circle cx="' + cx + '" cy="' + cy + '" r="' + r +
           '" fill="none" stroke="#2C2C2C" stroke-width="3"/>';
    svg += '</svg>';
  ...
  function checkAnswer() {
    ...
    if (ok) {
      ...
      showBanner('성공! ' + problem.num + '/' + problem.den + ' 맞아요!', 'ok');
      sounds.play('correct');
    } else {
      p1Card.classList.add('wrong');
      p2Card.classList.add('wrong');
      showBanner('아쉬워요! 정답은 ' + problem.num + '/' + problem.den, 'ng');
      sounds.play('wrong');
    }
  ...
  function nextRound() {
    ...
    var problem = problems[currentRound];
    renderPie(problem.num, problem.den);
