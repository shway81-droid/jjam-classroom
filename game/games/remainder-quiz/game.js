...
function generateQuestion() {
  // Division-remainder question: a ÷ b, find the remainder r.
  const b = difficulty === 'easy' ? rand(2, 5) : rand(2, 9);
  const q = difficulty === 'easy' ? rand(1, 9) : rand(2, 12);
  const r = rand(0, b - 1);
  const a = b * q + r;

  const answer = r;
  const expr   = `${a} ÷ ${b} 의 나머지 = ?`;

  // Build 3 distinct wrong choices from [0, max(b-1, r+4)], excluding answer.
  const wrongSet = new Set();
  const hi = Math.max(b - 1, r + 4);
  let guard = 0;
  while (wrongSet.size < 3 && guard < 200) {
    guard++;
    const w = rand(0, hi);
    if (w !== answer && w >= 0) wrongSet.add(w);
  }
  // Fallback: if not enough candidates exist, allow r+1..r+4.
  for (let k = 1; wrongSet.size < 3 && k <= 4; k++) {
    const w = r + k;
    if (w !== answer && w >= 0) wrongSet.add(w);
  }
