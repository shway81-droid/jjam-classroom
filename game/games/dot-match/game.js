...
const DOT = '#FF7043';           // dot color
const INK = '#2C2C2C';           // border
const MIN_TARGET = 4;            // fewest dots on the target card
const MAX_TARGET = 9;            // most dots on the target card
// candidate dot slots (4 cols × 3 rows) inside a 100×100 card
const SLOTS = [];
for (let r = 0; r < 3; r++) {
  for (let c = 0; c < 4; c++) SLOTS.push({ x: 20 + c * 20, y: 26 + r * 24 });
}
...
let scores        = [];
let questionLog   = [];
let currentQuestion = null;  // { instruction, choices:[svg], answerIdx, answerLabel }
let dqSet         = new Set();
let phase         = 'idle';
...
function dotCardSvg(count) {
  ...
  for (const s of slots) {
    const jx = s.x + rand(-4, 4);
    const jy = s.y + rand(-4, 4);
    dots += `<circle cx="${jx}" cy="${jy}" r="7.5" fill="${DOT}" stroke="${INK}" stroke-width="1.6"/>`;
  }
  return `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" class="pattern-svg">` +
    `<rect x="1.5" y="1.5" width="97" height="97" rx="8" fill="#FFFDF7" stroke="${INK}" stroke-width="3"/>` +
    dots + `</svg>`;
}

function buildQuestion() {
  const target = rand(MIN_TARGET, MAX_TARGET);
  const counts = [target];
  while (counts.length < 4) {
    const c = rand(3, 10);
    if (!counts.includes(c)) counts.push(c);
  }
  const order = shuffle(counts.map((n, i) => ({ n, correct: i === 0 })));
  const choices = order.map(o => dotCardSvg(o.n));
  const answerIdx = order.findIndex(o => o.correct);

  const instruction = `🔴 점이 <b>${target}개</b>인 카드를<br>가장 먼저 찾아 콕!`;
...
function populateAnswers(question) {
  for (let i = 0; i < playerCount; i++) {
    ...
    btns.forEach((btn, j) => {
      btn.innerHTML = question.choices[j];
      btn.className  = 'answer-btn';
      btn.disabled   = false;
      if (dqSet.has(i)) {
...
function resolveQuestion(winnerIdx, winBtn) {
  ...
  if (winnerIdx >= 0) {
    ...
    questionLog.push({
      answer:    currentQuestion.answerLabel,
      winnerIdx,
      dqSet:     new Set(dqSet),
      timedOut:  false,
    });
...
function nextQuestion() {
  ...
  currentQuestion = gameQuestions[questionIdx];
  questionIdx++;
  dqSet   = new Set();
  phase   = 'idle';
  ...
  problemExpr.innerHTML = currentQuestion.instruction;
