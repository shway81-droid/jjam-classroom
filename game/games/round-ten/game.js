...
// ── Problem generation ───────────────────────────────────────
// "반올림 척척": show a number, pick the value rounded to the NEAREST TEN.
function generateQuestion() {
  // Pick source number n. Exclude multiples of 10 (trivial) and units-digit 5
  // (ambiguous half-rounding) so the nearest-ten answer is always unambiguous.
  const lo = difficulty === 'easy' ? 11  : 101;
  const hi = difficulty === 'easy' ? 99  : 999;
  let n;
  do {
    n = rand(lo, hi);
  } while (n % 10 === 0 || n % 10 === 5);

  const answer = Math.round(n / 10) * 10;   // nearest ten (safe: units !== 5)
  const expr   = `${n}  →  ?`;              // the prompt shown big

  // Build 3 distractor choices that are ALSO multiples of ten, distinct from
  // the answer and each other and >= 0.
  const candidates = [
    answer - 20, answer - 10, answer + 10,
    answer + 20, answer + 30, answer - 30,
  ];
  const wrongSet = [];
  shuffle(candidates).forEach(c => {
    if (c >= 0 && c !== answer && !wrongSet.includes(c) && wrongSet.length < 3) {
      wrongSet.push(c);
    }
  });
...
function showResult() {
  ...
  resultTableBody.innerHTML = questionLog.map((q, ri) => {
    ...
    return `<tr>
      <td>${ri + 1}</td>
      <td style="font-size:0.8rem;white-space:nowrap">${q.expr.replace('  →  ?','').replace(' →  ?','')}</td>
