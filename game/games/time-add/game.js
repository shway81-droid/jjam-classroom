...
let scores         = [];
let roundLog       = [];
let currentProblem = null;   // { prompt, answerStr, choices[] }
let currentChoices = [];
let dqSet          = new Set();
let phase          = 'idle';
...
function minutesToTuple(totalMin) {
  ...
  const h0 = Math.floor(m / 60);      // 0..11
  const mm = m % 60;
  return { h: h0 === 0 ? 12 : h0, mm: mm };
}

function fmtTuple(t) {
  return t.mm === 0 ? `${t.h}시` : `${t.h}시 ${t.mm}분`;
}
...
function buildProblem() {
  ...
  const startM   = randInt(0, 11) * 5;                 // 0,5,...,55
  const addM     = randInt(1, 11) * 5;                 // 5,10,...,55
  const startTot = (startH % 12) * 60 + startM;
  const endTot   = startTot + addM;

  const answerT   = minutesToTuple(endTot);
  const answerStr = fmtTuple(answerT);

  const startStr = startM === 0 ? `${startH}시` : `${startH}시 ${startM}분`;
  const prompt   = `${startStr}에서 ${addM}분 후는?`;

  // 오답 3개: 도착 시각 근처의 그럴듯한 시각(정답·서로 중복 금지)
  const offsets = shuffle([-60, 60, -30, 30, -15, 15, -10, 10, -5, 5, -45, 45]);
  const choiceTuples = [answerT];
  for (const off of offsets) {
    if (choiceTuples.length >= 4) break;
    const t = minutesToTuple(endTot + off);
    if (choiceTuples.some(c => tupleEq(c, t))) continue;
    choiceTuples.push(t);
  }
  // 안전장치: 만에 하나 4개가 안 차면 임의로 채움
  let guard = 0;
  while (choiceTuples.length < 4 && guard++ < 100) {
    const t = minutesToTuple(endTot + randInt(1, 11) * 5 * (Math.random() < 0.5 ? 1 : -1));
    if (!choiceTuples.some(c => tupleEq(c, t))) choiceTuples.push(t);
  }

  return {
    prompt: prompt,
    answerStr: answerStr,
    choices: shuffle(choiceTuples.map(fmtTuple)),
  };
}
...
function buildRounds(n) {
  ...
  while (rounds.length < n && guard++ < 500) {
    ...
    if (seen.has(p.prompt)) continue;   // 같은 문제 중복 방지
    seen.add(p.prompt);
    rounds.push(p);
  }
  return rounds;
...
function loadRound() {
  ...
  wordDisplay.textContent = currentProblem.prompt;
...
function showResult() {
  ...
  roundLog.forEach((log, idx) => {
    ...
    let cells = `<td style="text-align:left;font-size:0.78rem;">${idx + 1}. ${log.prompt.replace('는?', '')}= ${log.answerStr}</td>`;
