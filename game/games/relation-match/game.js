...
let roundIdx       = 0;
let scores         = [];
let roundLog       = [];    // { label, winnerIdx(-1=timeout), dqPlayers[], timedOut }
let currentPair    = null;  // { emoji, word, answer }
let currentChoices = [];    // 4 answer strings for this round (includes correct)
let dqSet          = new Set();
let phase          = 'idle';
...
function renderPrompt(container, pair) {
  container.innerHTML = `
    <span class="prompt-emoji">${pair.emoji}</span>
    <span class="prompt-word">${pair.word}</span>
  `;
}
...
function emojiSvg(emoji) {
  return `<svg viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
    <rect x="3" y="3" width="54" height="54" rx="14" fill="#fff" stroke="#2C2C2C" stroke-width="3"/>
    <text x="30" y="40" text-anchor="middle" font-size="30"
...
function renderIntroFlags() {
  ...
  shuffle(ALL_PAIRS).slice(0, 3).forEach(pair => {
    const wrap = document.createElement('div');
    wrap.className = 'intro-pair-thumb';
    wrap.innerHTML = emojiSvg(pair.emoji);
...
function handleAnswerTap(playerIdx, chosenName, btn) {
  ...
  const correct = (chosenName === currentPair.answer);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentPair.answer) {
  ...
  roundLog.push({
    label: `${currentPair.word} - ${currentPair.answer}`,
    winnerIdx,
    dqPlayers: [...dqSet],
    timedOut: false,
  });
...
function handleTimeout() {
  ...
  problemStatus.textContent = `시간 초과! 정답: ${currentPair.answer}`;
...
function loadRound() {
  ...
  renderPrompt(flagDisplay, currentPair);
