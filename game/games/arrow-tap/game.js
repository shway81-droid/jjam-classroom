...
const ALL_ARROWS = [
  ...
  { name: '왼쪽',        key: 'left',       angle: -90 },
  { name: '오른쪽',      key: 'right',      angle: 90 },
  { name: '오른쪽 위',   key: 'up-right',   angle: 45 },
  { name: '오른쪽 아래', key: 'down-right', angle: 135 },
  { name: '왼쪽 아래',   key: 'down-left',  angle: -135 },
  { name: '왼쪽 위',     key: 'up-left',    angle: -45 },
];

// 라운드 풀: 32개 (게이트 1번 — 30개 이상)
function buildRoundPool() {
  const pool = [];
  for (let i = 0; i < 4; i++) {
    ALL_ARROWS.forEach(a => pool.push(a));
  }
  return pool;
}
const ROUND_POOL = buildRoundPool();

// 큰 화살표 SVG (상단 디스플레이)
function arrowDisplaySVG(angle) {
  return `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
    <g transform="rotate(${angle} 50 50)">
      <rect x="40" y="22" width="20" height="46" fill="#2C2C2C"/>
      <polygon points="20,30 80,30 50,5" fill="#2C2C2C"/>
    </g>
  </svg>`;
}

// 버튼용 작은 화살표 SVG
function arrowBtnSVG(angle, bgColor) {
  return `<svg viewBox="0 0 110 70" xmlns="http://www.w3.org/2000/svg">
    <rect x="2" y="2" width="106" height="66" rx="14" ry="14"
          fill="${bgColor}" opacity="0.95" stroke="#2C2C2C" stroke-width="3"/>
    <g transform="rotate(${angle} 55 35)">
      <rect x="49" y="20" width="12" height="26" fill="#fff"/>
      <polygon points="37,24 73,24 55,9" fill="#fff"/>
    </g>
  </svg>`;
}
...
function makeChoices(correctKey) {
  const others = shuffle(ALL_ARROWS.filter(a => a.key !== correctKey));
  const wrong = others.slice(0, 3);
  const correct = ALL_ARROWS.find(a => a.key === correctKey);
...
function renderIntroArrows() {
  ...
  const sample = [
    { angle: 0,   color: '#B3E5FC', stroke: '#0288D1' },
    { angle: 90,  color: '#FFCDD2', stroke: '#E53935' },
    { angle: -90, color: '#C8E6C9', stroke: '#388E3C' },
  ];
  sample.forEach(s => {
    const wrap = document.createElement('div');
    wrap.className = 'intro-arrow-thumb';
    wrap.innerHTML = `<svg viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">
      <rect x="3" y="3" width="74" height="74" rx="14" fill="${s.color}" stroke="${s.stroke}" stroke-width="3"/>
      <g transform="rotate(${s.angle} 40 40)">
        <rect x="33" y="22" width="14" height="30" fill="#2C2C2C"/>
        <polygon points="18,28 62,28 40,8" fill="#2C2C2C"/>
      </g>
    </svg>`;
...
function populateAnswerBtns() {
  for (let i = 0; i < playerCount; i++) {
    ...
    currentChoices.forEach(arrow => {
      const btn = document.createElement('button');
      btn.className = 'answer-btn';
      btn.dataset.player = i;
      btn.dataset.choice = arrow.key;
      btn.setAttribute('aria-label', `P${i + 1} ${arrow.name}`);
      btn.innerHTML = arrowBtnSVG(arrow.angle, PLAYER_CONFIG[i].dot);

      onTap(btn, () => handleAnswerTap(i, arrow.key, btn));
...
function handleAnswerTap(playerIdx, chosenKey, btn) {
  ...
  const correct = (chosenKey === currentArrow.key);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentArrow.key) {
...
function handleTimeout() {
  ...
  problemStatus.textContent = `시간 초과! 정답: ${currentArrow.name}`;
...
function loadRound() {
  ...
  currentChoices = makeChoices(currentArrow.key);
  dqSet        = new Set();

  questionCounter.textContent = `${roundIdx + 1} / ${TOTAL_ROUNDS}`;
  flagDisplay.innerHTML = arrowDisplaySVG(currentArrow.angle);
