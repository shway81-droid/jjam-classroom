/* games/second-big/game.js — 두 번째로 큰 수 */
...
// 문제 생성: 서로 다른 수 n개(n>=4). 정답 = '두 번째로 큰 수'.
// 값이 모두 다르므로 1등·2등·3등이 명확히 구분된다(가운데 수와도 다름).
...
function btGeneratePrompt(diff) {
  ...
  while (true) {
    ...
    values = [];
    const used = new Set();
    // 서로 다른 값들을 뽑는다 (동률 원천 차단 → 1·2·3등 명확)
    let guard = 0;
    ...
    if (values.length === n) break; // n개 모두 서로 다름
    if (attempts > 200) {
      values = [];
  ...
  const answerIdx = values.indexOf(secondVal);
...
(function() {
  ...
  if (el) {
    el.innerHTML = `<svg viewBox="0 0 220 110" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="85" y="32" width="50" height="46" rx="10" fill="#FFE0B2" stroke="#2C2C2C" stroke-width="3"/>
      <text x="110" y="64" text-anchor="middle" font-size="22" font-weight="900">71</text>
      <text x="110" y="22" text-anchor="middle" font-size="11" font-weight="900" fill="#9E9E9E">가장 큼</text>
      <rect x="150" y="32" width="50" height="46" rx="10" fill="#A5D6A7" stroke="#2E7D32" stroke-width="3"/>
      <text x="175" y="64" text-anchor="middle" font-size="22" font-weight="900" fill="#1B5E20">58</text>
      <text x="175" y="22" text-anchor="middle" font-size="11" font-weight="900" fill="#2E7D32">두 번째!</text>
    </svg>`;
  }
})();
...
function btStartGame() {
  ...
  btBuildZones();
  btBuildScoreBar();
  btQuestionCounter.textContent = '두 번째로 큰 수를 터치!';
