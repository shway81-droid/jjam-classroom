/* ===================================================================
   짬짬이 사이트가 공유하는 파일의 이탈 감지·동기화
   ===================================================================
   폴더는 여섯이지만 아래 파일들은 글자 하나까지 같아야 한다. 헤더의 자매 사이트
   ...
   왜 한 벌로 줄이지 않았나
     ...
     **빌드 단계 없음**이 원칙이고 심링크는 윈도우 체크아웃에서 말썽이다.
     ...
     그래서 파일은 여섯 벌로 두되, **손으로 고치는 것은 game/ 한 벌**로 정했다.
     나머지 다섯은 이 스크립트가 만든다. CI 가 --check 로 어긋남을 막는다.

   상류는 `game/` 이다 (저장소를 합치기 전 jjam 이 상류였던 것을 그대로 잇는다).

   쓰는 법
     node scripts/sync-shared.mjs           game/ 내용으로 나머지 다섯을 맞춘다
...
const SOURCE = 'game';
const TARGETS = ['quiz', 'video', 'story', 'word', '1min'];

// 여섯 폴더에서 동일해야 하는 파일 (폴더 기준 상대경로).
...
if (CHECK) {
  ...
  console.log(`✅ 공통 파일 ${SHARED.length}종이 여섯 폴더에서 모두 일치 (${same}건 확인)`);
} else {
  console.log(
    copied
