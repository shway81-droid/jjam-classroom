...
### 게임·퀴즈는 실제로 켜서도 확인합니다

위 명령은 "파일이 제대로 생겼나"만 봅니다. 게임이 **실제로 돌아가는지**는 따로
확인합니다 — 크로미움으로 띄워 인트로 → PLAY → 라운드 → 결과화면까지 자동
플레이하고 콘솔 에러가 0인지 봅니다.

```bash
cd game                                   # 또는 quiz
npm run verify:browser -- again-tap       # 게임 하나
npm run verify:browser -- --all           # 105종 전부 (십수 분)
```

CI(`게임 브라우저 실동작 검증`·`퀴즈 브라우저 실동작 검증`)가 같은 것을 돕니다.
PR 에서는 **그 PR 이 건드린 게임만**, main 푸시와 매주 월요일에는 전체를 돕니다.
`shared/style.css`·`shared/engine.js` 를 건드리면 전 게임이 로드하는 파일이므로
전체를 돕니다.

## 공통 파일은 `game/` 에서만 고칩니다

헤더의 자매 사이트 바로가기(`shared/jjam-switcher.js`)와 웹폰트는 여섯 폴더에서
같아야 합니다. **`game/` 이 상류**이고, 나머지 다섯은 아래 명령으로 따라옵니다.

```bash
node scripts/sync-shared.mjs           # game/ 내용으로 맞추기
node scripts/sync-shared.mjs --check   # 어긋난 곳 확인 (CI 게이트와 같은 명령)
```

`quiz/`·`video/`·`story/`·`word/`·`1min/` 안의 그 파일들은 생성물입니다. 손으로 고치면
CI 가 막습니다.

## 배포는 아직 기존 저장소에서 합니다

위 표의 주소는 **지금도 기존 저장소 여섯 곳이 서비스하고 있습니다.**
...
그래서 이 저장소는 **소스**만 맡고, 발행은 기존 여섯 곳이 그대로 맡습니다.
기존 저장소에서는 고치지 마세요 — 발행이 이곳 내용으로 덮어씁니다.

**반영은 자동입니다.** 사이트 폴더를 고쳐 `main` 에 머지하면 `발행 신호 보내기`
워크플로가 바뀐 폴더의 저장소에만 신호를 보내고, 몇 분 안에 사이트에 나옵니다.
신호가 막히더라도 각 저장소의 **하루 한 번 크론(KST 새벽 4시대)** 이 안전망으로
남아 있어 사이트가 오래 묵지 않습니다.
