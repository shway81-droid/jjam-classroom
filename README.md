...
헤더의 자매 사이트 바로가기(`shared/jjam-switcher.js`)와 웹폰트는 다섯 폴더에서
같아야 합니다. **`game/` 이 상류**이고, 나머지 넷은 아래 명령으로 따라옵니다.
...
`quiz/`·`video/`·`story/`·`word/` 안의 그 파일들은 생성물입니다. 손으로 고치면
CI 가 막습니다.
...
그래서 이 저장소는 **소스**만 맡고, 발행은 기존 다섯 곳이 그대로 맡습니다.
